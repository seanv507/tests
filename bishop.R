...
summary(polyfit_raw)
# polyfit_raw_ridge<-lm.ridge(y~poly(x,poly_order,raw=T),data=training_data,lambda=10)
...
#pred_raw_data<-poly(x=x_long, poly_order,raw=T)
#coef_polyfit_raw_ridge<-coef(polyfit_raw_ridge)
#pred_polyfit_raw_ridge <- polyfit_raw_ridge$ym + rowSums(pred_raw_data %*% (coef_polyfit_raw_ridge[1:poly_order+1]))
